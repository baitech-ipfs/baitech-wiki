...
Baitech Storage API 的 Base URL 为：https://api.baitech-ipfs.net
...
```bash
curl -X GET "https://api.baitech-ipfs.net/ping" \
...
```
curl -X GET "https://api.baitech-ipfs.net/endpoint/list" \
  -H "Authorization: Bearer <YOUR_API_KEY>" \
  -H "Content-Type: application/json"
```
 ...
 [
   {
     "id": 1,
     "name": "上海",
     "apiHost": "https://api-sh.baitech-ipfs.net"
   },
   ...
]
```
...
申请 Access Token：
```sh
curl -X GET "https://api.baitech-ipfs.net/oauth/accessToken" \
  -H "Authorization: Bearer <YOUR_API_KEY>" \
  -H "Content-Type: application/json"
```
...
```sh
curl -X POST "https://api-sh.baitech-ipfs.net/pinning/pinFile" \
